...
| v27 | 2026-03-01 | DistilBERT | 4 (early stop, best@4) | 2e-5 | 32 | 0.390 | 0.655 | +3,680 separated-llm: ucc (150 texts), civil_comments (100 texts), extreme-adco (118 texts) × 10 dims. DB: 22,304 texts, 94,041 scores. | `--drop-proxy-dims`. Same config as v23. **REGRESSED — not promoted.** |
| v28 | 2026-03-06 | DistilBERT | 6 (early stop, best@3) | 2e-5 | 32 | 0.412 | 0.678 | Same DB as v27 + rescore-368 re-ingested (3,680 sep-llm, cleaned labels). 17,708 train. | No `--drop-proxy-dims` (B3 diagnostic baseline). **Not promoted** (TE=0.762 < v23 0.795). |
...
| v32 | 2026-03-07 | DistilBERT | 8 (early stop, best@5) | 2e-5 | 32 | 0.385 | 0.676 | +700 TE texts (unlabeled pool F3b; score=5=9.9%; mean=4.81; esconv/prosocial excluded from F3b). ~15,559 train. | `--drop-proxy-dims`. B3 F3b — TE expansion. TE=0.739 (−0.034 vs v31, counterintuitive regression). HI −0.075. **REJECTED.** |
| v33 | 2026-03-07 | DistilBERT | early stop, best@6 | 2e-5 | 32 | 0.390 | 0.672 | +350 TE texts (F4: 200 prosocial + 150 esconv, targeted at held-out source gaps). ~15,909 train. | `--drop-proxy-dims`. B3 F4 — distribution-gap fix. TE=0.742 (+0.003 vs v32, within SE(r)≈0.10 noise). 5th consecutive TE rejection. **REJECTED. B3 CLOSED.** |
| v34 | 2026-03-07 | DistilBERT | early stop, best@6 | 2e-5 | 32 | 0.406 | 0.678 | +260 synthetic AD texts (formal authority range augmentation, generate_ad_batch.py). 15,803 train / 2,306 val / 2,377 test. | `--drop-proxy-dims`. AD augmentation (§69). AD +0.067 vs v33 but 4 dims regressed; overall 0.678 < v23 0.684. **REJECTED.** |
...
| v36 | 2026-03-08 | DistilBERT | 10 (best@8) | 2e-5 | 32 | 0.416 | 0.680 | +350 HI augmentation texts (Opus, HI only). 24,639 texts, 106,703 scores. | `--drop-proxy-dims`. HI diagnostic: HI=0.709 (v35=0.714, −0.005). HI augmentation did NOT improve — cross-scorer concordance failure explains (Opus HI +0.82 bias vs Sonnet). **DIAGNOSTIC ONLY — not promoted.** |
| v37 | 2026-03-08 | DistilBERT | 10 (best@10) | 2e-5 | 32 | 0.387 | **0.639** | Opus remediation: 999 texts re-scored with Sonnet (9,990 new scores, separated-LLM). 16,702 train / 2,392 val / 2,466 test. | `--drop-proxy-dims`. Clean Sonnet-only labels. Δ vs v35 = −0.041, Fisher z=0.50, p=0.617 (NS). CC −0.109, CO −0.106 (flagged for monitoring). 3,035s training. **PRODUCTION.** Hetzner deployed. Calibration: isotonic-v2-2026-03-08. |

**Notes on v32–v34 (B3 F3b/F4 + AD augmentation):** v32 (700 TE texts, F3b) showed counterintuitive TE regression −0.034 vs v31 despite high-quality labels (9.9% score=5). v33 (350 TE texts, distribution-rebalanced F4) recovered TE only +0.003 — within SE(r)≈0.10 noise. Five consecutive TE failures (v29, v31–v33 + v30 single-task ceiling) closed B3: SE(r)≈0.10 at n=99 makes sub-±0.10 TE gains unresolvable. v34 tested AD augmentation (260 synthetic formal authority texts): AD +0.067 vs v33 but overall 0.678 < v23 0.684, 4 dims regressed. Cross-scorer concordance study run during this period revealed v35 Opus labels are not interchangeable with Sonnet (ICC=0.495) — see §72.
...
**IMPORTANT — eval correction (2026-03-01):** All held-out_r values in the table above were measured with `max_length=256` in eval_held_out.py, despite training using `max_length=128`. This inflated all held-out_r by approximately 0.012 (the extra context window gave the model information it didn't see during training). Bug fixed in eval_held_out.py, calibrate.py, and distill.py PSQDataset. v23 corrected: 0.696→**0.684**. v27 measured with corrected eval: **0.655**. Historical values in the table are internally consistent but systematically ~0.012 high.
