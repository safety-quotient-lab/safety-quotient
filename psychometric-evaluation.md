...
**Status:** v21 DistilBERT — test_r=0.504, held-out_r=0.630. Scoring experiments complete: all three interventions REJECTED (halo-awareness reversed after g-factor structural analysis — g-factor is real co-variation, not scorer artifact). Test-retest r=0.804 (excluding AD). 4 criterion validity studies. Hierarchical model: PSQ → clusters → dimensions.
...
**Update (2026-02-28, scoring experiments):** Three controlled experiments tested scorer-level interventions for halo reduction. Halo-awareness instruction initially met pre-registered adoption criteria (+26.4% within-text SD, eigenvalue 78.4%→68.6%, all ρ ≥ 0.79, criterion validity AUC=0.971). However, post-hoc structural analysis revealed the g-factor is primarily a **range/extremity effect** — not scorer halo. Extreme texts (g < 3 or g > 7, n=232) show EV1=82.8% with uniform PC1 loadings (SD=0.023); middle texts (g 4-6, n=1,447) show EV1=38.7% with structured loadings (SD=0.117). The instruction's individual score changes (mean |Δ|=0.217) are below the test-retest noise floor (MAD=0.54), and ~1/3 of the headline improvement is attributable to CC bias and CO decoupling. **All three interventions REJECTED.** The g-factor is the PSQ at its broadest hierarchical level — the correct approach is structural (middle-g text enrichment for training data), not scorer-level. See `scoring-experiments.md` for full results and `distillation-research.md` §51 for the structural analysis.
